...
def _summary(mol, basis_name: str) -> None:
    ...
    print(
        f"  {basis_name:<12s}  n_basis={basis.n_basis:>3d}  "
        f"shells={len(basis.shells):>2d}  "
        f"O:{o_ls}  H:{h_ls[:len(h_ls)//2]} (each)"
    )


def main() -> None:
    water = _water()
    print(f"Water: {water.elements}, n_electrons={water.n_electrons}\n")
    print("Basis summaries:")
